...
class ChessGUI_pygame:
    ...
    def GetPlayerInput(self,board,currentColor):
        ...
        while not fromSquareChosen or not toSquareChosen:
            squareClicked = []
            esc_quit = raw_input('esc, quit: ')
            # Replace Input with alex output
            cmd_r = int(raw_input("Row: "))
            cmd_c = int(raw_input("Column: "))
            squareClicked = (cmd_r, cmd_c)
            if squareClicked[0]<0 or squareClicked[0]>7 or squareClicked[1]<0 or squareClicked[1]>7:
                ...
                print('Not a valid chess square!')

            if 'esc' in esc_quit:
                fromSquareChosen = 0
                fromTuple = []
            if 'quit' in esc_quit: #the "x" kill button
                pygame.quit()
                sys.exit(0)

            if not fromSquareChosen and not toSquareChosen:
                # self.Draw(board)
                possibleDestinations = self.Rules.GetListOfValidMoves(board,currentColor,squareClicked)
