...
def home_view(request):
    """View for displaying the home template."""
    return {}


@view_config(route_name='registration', renderer='../templates/registration.jinja2')
def registration_view(request):
    """View for displaying the form for registering a new user."""
...
def login_view(request):
    """View for displaying the form for logging in the user."""
    if request.method == "POST" and request.POST:
        if request.POST["username"] and len(request.POST["username"].split()) > 1:
            new_name = request.POST["username"].split()
            new_name = '_'.join(new_name)
...
def add_smack(request):
    """View for displaying a form for adding smack talk to the data base."""
    if request.method == "POST" and request.POST:
...
def smack_json(request):
    """View for displaying JSON from the database."""
