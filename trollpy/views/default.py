...
def registration_view(request):
    ...
    if request.method == "POST" and request.POST:
        ...
        return HTTPFound(request.route_url(
            'profile', userid=new_name),
            headers=auth_head)
    if request.authenticated_userid:
        return HTTPFound(
            request.route_url('profile', userid=request.authenticated_userid))
    else:
        user = None
    return {"user": user}
...
def profile_view(request):
    ...
    profile = request.dbsession.query(
        User).filter_by(username=theuserid).first()
    if request.method == 'POST' and request.POST:
...
@view_config(
    route_name='add_smack',
    renderer='../templates/add_smack.jinja2',
    permission="add",
    require_csrf=True
)
def add_smack(request):
    ...
    if request.method == "POST" and request.POST:
        ...
        mp3_id = request.dbsession.query(
            KillScore).filter_by(
            statement=request.POST['statement']).first().id
        tts = gTTS(text=request.POST['statement'], lang='en')
        tts.save("hello.mp3")
        current_place = os.path.dirname(
            os.path.abspath(__file__))[:-13] + 'hello.mp3'
        print(current_place)
...
@view_config(
    route_name='del_smack',
    permission="view",
    require_csrf=False
)
def del_smack(request):
    """Delete smack-talk from the database."""
...
@view_config(
    route_name='api_smack',
    renderer='json',
    permission="view",
    require_csrf=False
)
def smack_json(request):
    """Return smack DB as JSON."""
...
def user_board_json(request):
    """Return user board as JSON."""
    theuserid = request.matchdict['userid']
    user = request.dbsession.query(User).filter_by(username=theuserid)
    stuff = user.first().to_json()
    mp3_id = request.dbsession.query(
        KillScore).filter_by(
        statement=user.first().to_json()['trollspeak']).first().id
    stuff['id'] = mp3_id
    return stuff


@view_config(
    route_name='make_move',
    permission="view",
    require_csrf=False
)
def make_move(request):
    ...
    if request.method == "POST" and request.POST:
        ...
        if not board_winner[1]:
            user.update({
                'winner': board_winner[1],
                'board': board_winner[0]}
            )
...
def send_mp3(request):
    ...
    data = request.dbsession.query(
        Audio).filter_by(id=mp3_id).first().binary_file
